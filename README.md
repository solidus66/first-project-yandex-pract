...
- `git status` - Показывает текущий статус рабочего каталога и индекса
- `git log` - Отображает историю коммитов
- `git add <filename>` - Добавляет указанные файлы в индекс
- `git commit -m "Сообщение к коммиту"` - Создает новый коммит с указанным сообщением
